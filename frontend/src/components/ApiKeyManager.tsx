...
export function ApiKeyManager() {
  ...
  const handleCreate = async () => {
    ...
    try {
      const resp: RawKeyResponse = await post("/admin/keys", {
        name: newName,
        permissions: newPerm,
      });
  ...
  return (
    <div>
      ...
      >
        <div class="grid grid-cols-2 gap-2 mb-2">
          <label class="block">
            Name
            <input
              class="w-full border border-hestia-border rounded p-2"
              value={newName}
      ...
      {rawKeyDisplay && (
        <div
          class="bg-hestia-primary/10 p-4 mt-4 rounded"
          style="background:rgba(1,114,173,0.1)"
        >
          <strong>Save now — this key cannot be viewed again!</strong>
          <code class="block mt-2 break-all font-mono">{rawKeyDisplay}</code>
          <button
            class="mt-2 px-3 py-1 text-sm border border-current rounded bg-transparent hover:bg-hestia-primary hover:text-white"
      ...
      {keys.length === 0 ? (
        <p class="text-hestia-text-muted">No keys created yet.</p>
      ) : (
        keys.map((key) => (
          <div
            key={key.id}
            class="mb-2 border border-hestia-border rounded p-3"
          >
            <div class="flex justify-between items-center">
              <div>
                <strong>{key.name}</strong>{" "}
                <em class="text-hestia-text-muted" style="font-style:normal">
                  ({key.permissions})
                </em>
